...
const COPIES = [
  ...
  ['rules/es.json', 'docs/vendor/es.json'],
  ['rules/de.json', 'docs/vendor/de.json'],
  ['rules/pt.json', 'docs/vendor/pt.json'],
];
...
// Keep the number on the landing page in step with the rules of every language (not the strict pack).
const count = ['en', 'fr', 'es', 'de', 'pt'].reduce((n, f) => n + JSON.parse(readFileSync(new URL(`rules/${f}.json`, root), 'utf8')).rules.length, 0);
